...
public static class StartupDiagnostics
{
    public static void Trace(string stage)
    {
        ...
        try {Directory.CreateDirectory(directory);File.AppendAllText(Path.Combine(directory,$"startup-{Environment.ProcessId}.txt"),$"{DateTimeOffset.UtcNow:O} {stage}\n");}catch{}
    }
    public static void Attach(Application app)
    {
        Trace("Application constructor entered");
        var directory=Environment.GetEnvironmentVariable("ZHELI_STARTUP_DIAGNOSTICS");
        if(string.IsNullOrWhiteSpace(directory))return;
